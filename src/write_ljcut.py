...
# Assuming the force field is LJ/CUTMORSE
...
def write_in(ffield, data_file):
    ...
    g.write('# Lj/CUT - generated by optimizer\n')
    g.write('units            ' + units + '\n')
    ...
    g.write('\n')
    g.write('pair_style       hybrid/overlay coul/pqeqgauss 0.00 20.00 lj/cut 20.0\n')  # Change later
    g.write('\n')
    ...
    g.write('#                       epsilon sigma \n')
    g.write('pair_coeff  * *  lj/cut 0 0 \n')
    for i in range(len(dict['VDW'])):
        temp = dict['VDW'][i]
        g.write('pair_coeff ' + str(temp[0]) + ' ' + str(temp[1]) + ' lj/cut ' + ' '.join(map(str, temp[2:])) + '\n')
    g.write('\n')
    g.write('pair_modify pair lj/cut mix geometric\n')
    g.write('# CHECK MIXING RULE FOR LJ/CUT\n')
    g.write('\n')
    """
    ...
    g.write('thermo_style     custom pe evdwl v_epqeq\n')

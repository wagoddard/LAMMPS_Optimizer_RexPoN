...
use Packages::General qw(FileTester LoadElements Permutate GetSoluteAtoms 
						CRadDegrees IsDecimal LoadFFs AddRingField AddElementField);
...
my ($ATOMS, $CONS, $HEADERS, $FILES, $PARMS, %ERRORS, $i, $BOX, $ffType, $ismesodna, $nocross);
my ($BONDS, $TORSIONS, $INVERSIONS, $ANGLES, @atmIndices, $totAtms);
my ($writeLabels, $MOLS, $QEqtmp);
...
&fixMesoDNAAtoms($ATOMS, $CONS) if ($ismesodna); #mesodna fix
...
&addQEqData($ATOMS,$PARMS->{QEq}, $QEqtmp);
print "Done\n";
...
&addPQEqOpt($PARMS) if ($PARMS->{QEq}{Opt} == 2);
...
&writeIsotopeMass($ATOMS, $PARMS, \*DATFILE);
close DATFILE;
...
sub addPQEqOpt {
	my ($parms) = $_[0];
	my ($i, $indx, $tmp, $curr, $qeqParm);

	for $i (keys %{ $parms->{VDW} }) {
		next if ($i eq "TYPE" or $i eq "counter" or !exists($parms->{QEq}{$i}));
		$curr = $parms->{VDW}{$i}{$i};
		$qeqParm = $parms->{QEq}{$i};
...
sub addQEqData {
	...
	$parms->{Opt} = $tmp->{Opt};
	$parms->{File} = $tmp->{File};
	return if ($parms->{Opt} == 0);

	if($parms->{Opt} == 1) {
		$parms->{DATA} = parseQEqFile($parms->{File});
	} elsif ($parms->{Opt} == 2) {
		$parms->{DATA} = parsePQEqFile($parms->{File});
	}

	for $i (keys %{ $atoms }) {
		$ele = $atoms->{$i}{ELEMENT}{SYMBOL};
		$ftype = $atoms->{$i}{FFTYPE};
		next if exists($parms->{$ftype});
		die "ERROR: No QEq paramaters found for atom $i (type: $ftype element: $ele)!\n"
			if (!exists($parms->{DATA}{$ele}));
		$parms->{$ftype} = $parms->{DATA}{$ele};
	}
}
...
sub parsePQEqFile {
	...
	while(<QEqPARM>) {
		chomp;
		if ($_ =~ /^\s*([a-zA-Z]+)\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)/) {
			$parm->{$1} = (
							{
								"P"     => $2,
								"CHI"   => $3,
...
sub createDatFileHeader {
	...
	my ($cos_alpha, $sin_alpha, $cos_beta, $sin_beta, $cos_gamma, $sin_gamma, $tmp);
	...
	for $index (sort numerically keys %{ $tmp }) {
		printf $datFile "%5d %8.4f", $i, $parms->{ATOMTYPES}{$tmp->{$index}}{MASS};
		printf $datFile "%-10s",' # ' . $parms->{ATOMTYPES}{$tmp->{$index}}{NAME} if ($writeLabels);
		print $datFile "\n";
		$i++;
	}
	($DATA, $count) = sortParmByIndex($parms->{VDW}, 1);
	if ($ffType != 5 and (scalar(keys %{ $parms->{VDW}{TYPE} }) == 1 and $DATA->{1}{Lammps}{name} ne "sw")) {
		print $datFile "\nPair Coeffs\n";
		for $index (1 .. $count) {
			printf $datFile "\n%5d ", $index;
...
sub printAtoms {
	...
	$fmt = "%8d %8d %10.5f %10.5f %10.5f %8d %11.5f %11.5f %5d %5d %5d %4d %4d %4d\n" 
...
sub updateParmIndex {
	my ($parms) = $_[0];
	my ($i, $count, $PLIST, $index, @tmp, $j, $l, $k, $lammpsType, $dflag);
	...
	for $j (@tmp) {
		if (! $parms->{ATOMTYPES}{$j}{USED}) {
			...
			delete $parms->{VDW}{$j};
			delete $parms->{QEq}{$j} if ($parms->{QEq}{Opt}>0);
			for $i (@tmp) {
		...
		for $i (keys %{ $parms->{VDW}{$j} }) {
			...
			for $l (keys %{ $parms->{VDW}{$j}{$i} }) {
				...
				if ($k->{TYPE} eq "LJ_6_12" and exists($parms->{PARMS}{is_tip4p})) { # tip4p fix
					...
					$k->{Lammps}{opts} = "";
				} elsif($parms->{QEq}{Opt} == 2) {
					$k->{Lammps}{name} = "lj/gromacs" if($k->{Lammps}{name} =~ /charmm/);
					$k->{Lammps}{name} =~ s/\/coul.*//;
				}
				if ($k->{IT} eq "hbond") {
...
sub writeSWfile {
	...
	close SW_FILE;
	for $i (keys %{ $parms->{ATOMTYPES} }) {
		next if (ref($parms->{ATOMTYPES}{$i}) ne "HASH" or !keys %{ $parms->{ATOMTYPES}{$i} });
...
sub getTorsionWeightFactor {
	...
	if ($#{ $FILES } > 0 and $ffType > 4 and $ffType != 6) {
		...
		return $PARMS->{PARMS}{"EXO_CYCLIC"}*$torsion->{"1_4scale"} 
			if (is5MemberRing($atom1, $j, \@tmp) and 1>2);
	} else {
		return 0;
	}
}
...
sub getValParent {
	...
	for $i (keys %{ $valList }) {
		next if (($i eq "counter") or ($i eq "TYPE") or (exists($valList->{$i}{IGNORE}) and $valList->{$i}{IGNORE}));
		if (keys %{ $valList->{$i} }) {
			$valid = 0;
...
sub addLammpsParms {
	...
	for $i (1 ... $angles->{counter}) {
		if ($#{ $ANGLES->{LIST}{$i}{ATOMS} } == 2) {
			if ($atoms->{ $angles->{LIST}{$i}{ATOMS}[0] }{ELENUM} == 1 and
				$atoms->{ $angles->{LIST}{$i}{ATOMS}[1] }{ELENUM} == 8 and
	...
	$parms->{PARMS}{HYBRID_OVERLAY} = 1 if ($parmHybrid eq "hybrid/overlay ");
	for $i (keys %{ $parms->{VDW} }) {
		next if (($i eq "TYPE") or ($i eq "counter"));
		for $k (keys %{ $parms->{VDW} }) {
			#next if ((($k eq $i) and (scalar keys %{ $parms->{VDW}{$i}{$k} } == 1)) or ($k eq "TYPE")  ($k eq "counter"));
			next if ($k =~ /type|counter/i);
			next if (! exists($parms->{VDW}{$i}{$k}));
			next if ($k eq $i and ! $parmHybrid);
	...
	$parms->{VDW}{TYPE}{sw} = "" if ($parms->{PARMS}{SW});
	#reax fix
	if ($ffType == 5) {
		$parms->{PARMS}{REAX} = 1;
		@tmp = ();
...
sub checkAtomTypes {
	...
	for $i (keys %{ $atoms }) {
		...
		if (exists($atomTypes->{$ffType})) {
			...
			$atomTypes->{$ffType}{ELENAME} = $atoms->{$i}{ELEMENT}{ELENAME};
			$atomTypes->{$ffType}{ELENUM} = $atoms->{$i}{ELEMENT}{ELENUM};
...
sub determineIfHybrid {
	...
	for $i (@tmp) {
		for $k (keys %{ $parms->{$i} }) {
			for $l (keys %{ $parms->{$i}{$k} }) {
				$parms->{$i}{$k}{$l}{PAIRMIX} = 1 if(!exists($parms->{$i}{$k}{$l}{PAIRMIX}));
				$vdwType{ $parms->{$i}{$k}{$l}{TYPE} } = $parms->{$i}{$k}{$l}{PAIRMIX};
			}
		}
	}
	...
	for $i (keys %vdwType) {
		...
		for $j (@tmp) {
			...
			next if (! $jType);
			for $k (@tmp) {
				next if ((exists($parms->{$j}{$k}) and keys %{ $parms->{$j}{$k} }) or
					(exists($parms->{$k}{$j}) and keys %{ $parms->{$k}{$j} }));
	...
	for $i (@tmp) {
		last if ($PARMS->{PARMS}{HYBRID_OVERLAY});
		for $k (keys %{ $parms->{$i} }) {
			@tmp1 = keys %{ $parms->{$i}{$k} };
			if(scalar(@tmp1) > 1) {
...
sub createLammpsClusterScript {
	...
	$scriptCmd = "$scriptFile -p $suffix -b $bgfFile -f $ffList -i in.${suffix} -d data.${suffix} -s ${suffix}.lammps.slurm > _createscript";
	system($scriptCmd);
...
sub setOpts {
	...
	$parms->{PARMS}{OPTIONS}{HYBRID_OVERLAP} = 1;
	$parms->{PARMS}{OPTIONS}{HYBRID_OVERLAP} = 0 if ($opts =~ /no hybrid overlap/i);
	...
	$hasCharge = 1 if ($parms->{QEq}{Opts}>0);
...
sub writeIsotopeMass {
	...
	return 0 if (!exists($parms->{PARMS}{is_tip4p}) || ! exists($parms->{PARMS}{OPTIONS}{ISOTOPES}) || $parms->{PARMS}{OPTIONS}{ISOTOPES} == 0);
...
sub init {
	my ($i, $FFILES, $inputFFType, $inputList, $inputStr, $count, $auxOpt);
	
	getopt('bfstylomx',\%OPTS);
	die &usage if ((! exists($OPTS{b}) and ! exists($OPTS{m})) or ! exists($OPTS{f}));
	#($bgfFile, $msiFile, $FF, $suffix, $inputStr, $reaxFF, $inputFFType, $writeLabels, $opts, $pQeQopt) = 
	#($OPTS{b},$OPTS{m},$OPTS{f},$OPTS{s},$OPTS{t},$OPTS{r},$OPTS{y},$OPTS{l},$OPTS{o}, $pQeQopt);
	($bgfFile, $msiFile, $FF,     $suffix, $inputStr, $inputFFType, $writeLabels, $opts,    $auxOpt) = 
	($OPTS{b},$OPTS{m},  $OPTS{f},$OPTS{s},$OPTS{t},  $OPTS{y},     $OPTS{l},     $OPTS{o}, $OPTS{x});
	...
	#reax option
	if ($FF =~ /reax/i and !defined($auxOpt)) {
	...
	if($opts =~ /qeq/i) {
		...
		$QEqtmp->{Opt} = 1;
		if ($opts =~ /pqeq/i) {
			$QEqtmp->{File} = "$Bin/../ff/pqeq.par";
			$QEqtmp->{Opt} = 2;
		}
		$QEqtmp->{File} = $1 if(defined($auxOpt) and $auxOpt =~ /(\S+)/);
	}

	$BONDS->{counter} = $ANGLES->{counter} = $TORSIONS->{counter} = $INVERSIONS->{counter} = 0;
	$totAtms = 0;
	print "Done\n";
	$ismesodna = 1 if ($ffType == 3);
...
usage: $0 -b/-m structureFile -f \"ff1 ff2...\" -s [suffix] -o [options] -t [sim template] -x [auxillary file]
	-b bgfFile: BGF file. Must have correct atom types
	-m msiFile: MSI file.
	...
	-x [auxillary file]: (optional) Specifies an auxillary file for either the ReaxFF forcfield
									or the QEq/PQEq parameters sets

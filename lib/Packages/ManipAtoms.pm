...
@EXPORT_OK = qw(ImageAtoms ScaleAtoms UnwrapAtoms FindElement CenterSystem GetAtmList 
				...
				RemoveMolLongBonds ReimageMolsInBox);
...
sub ReimageMolsInBox {
	my ($atoms, $bonds, $mols, $box, $select) = @_;
	my ($i, $j, $k, $cMol, $com, $del);

	for $i (keys %{ $select }) {
		$cMol = ${ $atoms->{$i}{MOLECULEID} };
		$com = CoM(GetAtmData($atoms, $mols->{$cMol}{MEMBERS}));
		$del = 0;
		for $j ("X", "Y", "Z") {
			while($com->{"${j}COORD"}<$box->{$j}) { $com->{"${j}COORD"} += $box->{$j}; $del++};
			while($com->{"${j}COORD"}>$box->{$j}) { $com->{"${j}COORD"} -= $box->{$j}; $del--};
			next if ($del == 0);
			for $k (keys %{ $mols->{$cMol}{MEMBERS} }) {
				$atoms->{$k}{"${j}COORD"} += $del*$box->{$j};
			}
		}
	}
}
...
sub RemoveMolLongBonds {
	my ($atoms, $bonds, $mols, $box, $select) = @_;
	my ($i, $j, $k, $del, $delp, $deln, $l, $dir);

	for $i (keys %{ $select }) {
		for $j (@{ $bonds->{$i} }) {
			for $k ("X", "Y", "Z") {
				$del = $atoms->{$i}{"${k}COORD"} - $atoms->{$j}{"${k}COORD"};
				$delp = $del + $box->{$k};
				$deln = $del - $box->{$k};
				$l = $j; $dir = 1;
				if (exists($atoms->{$j}{FIXED})) {$l = $i; $dir = -1; }
				if ($delp*$delp<$del*$del) { 
					$atoms->{$l}{"${k}COORD"} += -1*$dir*$box->{$k}; 
					$atoms->{$i}{FIXED} = $atoms->{$j}{FIXED} = 1;
				}
				if ($deln*$deln<$del*$del) { 
					$atoms->{$l}{"${k}COORD"} +=  1*$dir*$box->{$k}; 
					$atoms->{$i}{FIXED} = $atoms->{$j}{FIXED} = 1;
				}
			}
...
sub getMolList {
	...
	for $i (@{ $bonds->{$atomID} }) {
		...
		for $j (keys %{ $atoms->{$atomID} }) {
			...
			push @{ $atoms->{$atomID}{BONDATOM}{$atoms->{$i}{$j}} }, \%{ $atoms->{$i} } if(exists($atoms->{$i}{$j}));
			push @{ $atoms->{$i}{BONDATOM}{$atoms->{$atomID}{$j}} }, \%{ $atoms->{$atomID} };
		}

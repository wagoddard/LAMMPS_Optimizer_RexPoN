...
@EXPORT_OK = qw(createBGF GetBGFFileInfo addElement addCon addHeader sortByRes createMOL2 GetMSIFileInfo
				...
				insertHeaderRemark InsertMol H2others GetXYZFileInfo GetMOL2Header);
$VERSION = "1.00";
...
sub createBGF {
	...
	for $Atm_no (sort numerically keys %{ $AtmData }) {
		...
		$atom->{"LONEPAIRS"} = 0 if (! exists($atom->{"LONEPAIRS"}));
	...
	for $atom (sort numerically keys %{ $AtmData }) {
		...
		for $i (0 .. $#{ $CON->{$atom} }) {
			...
			if (exists($AtmData->{$atom}{ORDER})) {
				$val = 0;
				for $j (@{ $AtmData->{$atom}{ORDER} }) {
					$val += $j;
				}
				$val--;
				delete $AtmData->{$atom}{ORDER} if($val==$#{ $AtmData->{$atom}{ORDER} });
			}	
...
sub GetXYZFileInfo {
	my ($infile) = @_;
	my ($inText, $natom, $dostart, $moln, $icount, %FData, $rec, @HEADER);

	$moln = basename($infile);
	$moln =~ s/\w+$//;
	$dostart = $icount = 0;
	open INFILE, $infile or die "Cannot open $infile: $!\n";
	while (<INFILE>) {
		...
		if (!defined($natom)) {
			if($inText =~ /^(\d+)/) {
				...
				die "ERROR: Expected the number of atoms in 1st line. Got \"$inText\"\n";
			}
		} elsif(! $dostart) {
			push @HEADER, "DESCRP $moln";
			push @HEADER, "REMARK: $inText";
			$dostart = 1;
		} elsif($inText =~ /^\s*(\S+)\s+(\-?\d+\.\d+)\s+(\-?\d+\.\d+)\s+(\-?\d+\.\d+)/) {
			$icount++;
			$rec = (
					{
						"HEADER"   => "ATOM",
		...
		last if($icount == $natom);
	}
	close INFILE;

	die "ERROR: Expected $natom atoms but only read $icount!\n"
		if ($icount<$natom);
	return (\%FData,undef,\@HEADER);
}
...
sub createMOL2 {
	...
	for $atom (@atmList) {
		...
		for $bond (0 .. $#{ $BONDS->{$atom} }) {
			...
			$order = 1;
			if (exists($ATOMS->{$atom}{ORDER}) and $#{ $ATOMS->{$atom}{ORDER} } >= $bond) {

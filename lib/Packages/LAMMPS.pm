...
sub createMiddle {
	...
	if (exists($parms->{PARMS}{is_tip4p}) && exists($parms->{PARMS}{OPTIONS}{ISOTOPES}) && $parms->{PARMS}{OPTIONS}{ISOTOPES} == 1) {
		$middle .= " fix Isotopes NULL Isotopes";
	}
	...
	$middle .= "$parms->{MESODNA}" if ($parms->{MESODNA});
	if($parms->{PARMS}{USE_HBOND}) {
		...
		$middle .= "variable	n_hbond equal c_hb[1]\n";
		$middle .= "variable	E_hbond equal c_hb[2]\n";
		$middle .= "thermo_style		 custom etotal ke temp pe ebond eangle edihed eimp evdwl ecoul elong v_E_hbond v_n_hbond press vol\n";
		$middle .= "thermo_modify   line multi flush yes\n";
	}
	if(exists($parms->{PARMS}{REAX})) {
		...
		$middle .= "\nfix		charge all pqeq 1 0.0 10.0 1.0e-06\n";
	}
	return $middle;
}
...
#					"$parms->{PARMS}{cut_coul} $parms->{PARMS}{cut_vdw}";
		...
		if (scalar keys %{ $parms->{VDW}{TYPE} } > 1) {
			if (exists($parms->{PARMS}{HYBRID_OVERLAY})) {

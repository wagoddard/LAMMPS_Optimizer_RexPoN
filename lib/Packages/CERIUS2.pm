...
my $scripts_dir = "/ul/joppenhe/lammps_optimizer/lib"; #change here as necessary
my $ff_dir = "/ul/joppenhe/lammps_optimizer/lib/ff"; #change here as necessary
@ISA = qw(Exporter);
@EXPORT = ();
...
sub parseCerius2FF {
	...
	my (@torsions, $torsion_type, $inversion_type, $counter, $use_hb, $use_charge, $ty_tmp);
	my ($atom3, $bond_counter, $angle_counter, $torsion_counter, @tmp, $inversion_counter, $bool);
	my ($atom1, $atom2, $vdwType, $vdwDat, $i, $dihdr_scale, $CNV, $vdw_counter, $crossType, $rec);
	...
	while (<FORCEFIELD>) {
		...
		} elsif ($in_data =~ /^\s*(\S+)\s+(\d+\.\d+\e?\-?\d*.+)/ and ($which_var == 10)) {
			@tmp = split /\s+/, $2;
			next if ($#tmp < 2);
			$PARMS{PARMS}{QEq} = 1;
...
#				GetAbs(\@torsions);
				...
				$torsions[0] *= 2 if ($torsion_type eq "FOURIER");
...
sub saveCeriusFF {
	...
	my ($c, $shft_angle, $torsions, $counter, $element, $vdws, $i, $chrg);
	...
	for $i (@tmp) {
		...
		if (-e $i &&  -r $i && -T $i) {
			...
			if ($#FFILES == -1) {
				...
				$ffType = 3 if ($i =~ /mesodna/i);
				$ffType = 4 if ($i =~ /dreid/i);
				$ffType = 6 if ($i =~ /opls/i);
				$ffType = 7 if ($rec->{FFTYPE} eq "SW");
			}
		} elsif (uc($i) eq "GAFF") {
			$rec = (
						{
